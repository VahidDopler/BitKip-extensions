...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  if (message.type == 'exatractLinkswithSpecificString') {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      chrome.tabs.sendMessage(
        tabs[0].id,
        { order: "getLinks-speicfic-string", linkRegex: message.linkRegex },
        async function (response) {
          console.log(response.links);
          const sender = new Connector();
          // await sender.postMessage({
...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  if (message.type == 'exatractLinkswithSpecificRegex') {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      chrome.tabs.sendMessage(
        tabs[0].id,
        { order: "getLinks-speicfic-regex", linkRegex: message.linkRegex },
        async function (response) {
          console.log(response.links);
          const sender = new Connector();
          // await sender.postMessage({
...
chrome.action.setPopup({ popup: './extension.html' });
